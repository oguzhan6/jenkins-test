...
graph TD
    ...
    PHP -- "API die connectie maakt met database en de gegevens beschikbaar stelt" --> API
...
Vervolgens worden **SQL-queries** naar de database gestuurd:
- `SELECT` om data te lezen  
- `INSERT` om data te schrijven  

### 1.3 Metrics Collection Flow
De **cAdvisor container** heeft toegang tot de Docker-socket van de host en verzamelt continu live metrics (CPU, geheugen, enz.) van alle andere actieve containers.

cAdvisor stelt deze metrics beschikbaar via een intern **HTTP-endpoint** (`/metrics`).

De **Prometheus container** is geconfigureerd om dit endpoint periodiek te *scrapen* (uitlezen) en slaat de data op in zijn **time-series database**.

### 1.4 Metrics Visualisatie Flow
De gebruiker opent de webinterface van de **Grafana container** via poort **3000**.

Grafana, waarin **Prometheus** als databron is toegevoegd, stuurt **PromQL-queries** naar de Prometheus container (poort **9090**) om de opgeslagen metrics op te vragen.

Grafana rendert deze data in grafieken en panelen binnen het geconfigureerde dashboard.
...
#### Post-sectie
Bevat een **echo** statement om te voorkomen dat de services na de pipeline-run worden afgesloten, zodat de website live blijft.
